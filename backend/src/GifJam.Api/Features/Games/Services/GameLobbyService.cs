    ...
    GameTelemetry gameTelemetry) : IGameService
{
    ...
    public async Task<PlayerGameSnapshot> CreateLobbyWithPlayersAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (userIds.Count is < GameRules.MinimumPlayers or > GameRules.MaximumPlayers ||
            userIds.Distinct().Count() != userIds.Count)
        {
            throw new BadRequestException(
                "invalid_matchmaking_player_count",
                $"A matched lobby must contain between {GameRules.MinimumPlayers} and {GameRules.MaximumPlayers} unique players.");
        }
        ...
        await dbContext.SaveChangesAsync(cancellationToken);
        gameTelemetry.GameCreated(game.Code, game.TotalRounds);
        return await GetAsync(code, hostUserId, cancellationToken);
    }
    ...
    {
        var gameId = await gameRepository.FindIdAsync(code, cancellationToken);
        return gameId ?? throw GameNotFound();
    }

    private Task<Game> LoadGameAsync(
        Guid gameId,
        CancellationToken cancellationToken,
        bool tracking = true) =>
        gameRepository.LoadAsync(gameId, cancellationToken, tracking);
