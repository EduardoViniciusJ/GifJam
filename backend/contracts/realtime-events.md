...
| `VoteGif` | `gameCode`, `gifSubmissionId` | Stores one non-self GIF vote during `GifVoting`. |
...
Deadlines are UTC timestamps supplied by the server. Clients render timers locally but never advance phases themselves.
